...
import (
	...
	"github.com/mugnainiguillermo/bookstore_oauth-api/src/domain/access_token"
...
func (r *dbRepository) GetById(id string) (*access_token.AccessToken, *rest_errors.RestErr) {
	...
	if err := cassandra.GetSession().Query(queryGetAccessToken, id).Scan(
		...
		return nil, rest_errors.NewInternalServerError("error while trying to get by id", err)
	}
...
func (r *dbRepository) Create(at access_token.AccessToken) *rest_errors.RestErr {
	if err := cassandra.GetSession().Query(queryInsertAccessToken,
		...
		return rest_errors.NewInternalServerError("error during create access token", err)
	}
...
func (r *dbRepository) UpdateExpirationTime(at access_token.AccessToken) *rest_errors.RestErr {
	if err := cassandra.GetSession().Query(queryUpdateExpires,
		...
		return rest_errors.NewInternalServerError("error during expiration time update", err)
	}
